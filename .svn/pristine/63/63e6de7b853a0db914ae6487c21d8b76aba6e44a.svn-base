...
class HeroModel(BaseModel): 
	...
	@classmethod
	def getUserHerosById(self, uuid, id): 
		if(id== None or uuid== None):      
			return None
	
		try:
			#data= self.mongodb['hs_user_hero'].find_one({"uuid": uuid,"_id": id})
			data= self.mongodb['hs_user_hero'].find_one({"_id": id})
			return data
	...
	@classmethod
	def getSystemOrUserHerosById(self, uuid, id): 
		if(id== None or uuid== None):      
			return None
		...
		if id< 1000000:
			...
			data= self.getUserHerosById(uuid, id)

		if data!= None:
			job= data['job']
			job_info= self.getUserJobInfo(uuid, job)
			data['xp']= 0 
			data['level']= 0 
	...
	@classmethod
	def getUserJobInfo(self, uuid, job): 
		if(job== None or uuid== None):      
			return None
		
		try:
			data= self.mongodb['hs_user_job'].find_one({"uuid": uuid, "job": job})		
			return data
		except AutoReconnect:
			self.log.error("mongodb error, AutoReconnect!")
		
	@classmethod
	def getUserAllJobInfo(self, uuid): 
		if(uuid== None):      
			return None
		...
		try:
			...
			for j in self.mongodb['hs_user_job'].find({"uuid": uuid}):
				#make a dict
				ret[j['job']]= j
